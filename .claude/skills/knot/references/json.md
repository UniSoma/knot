...
| `data.deleted.path`  | `delete`, `document rm`                                | Where the file *was* — it is gone.        |
...
|-------------------------|-------------------------------------------------------------------------------------------------|------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| `not_found`             | Strict-resolved id matched no ticket (live or archive).                                         | —                            | `show`, `start`, `status`, `close`, `reopen`, `delete`, `dep` (from), `undep` (from), `link` (either), `unlink` (from), `add-note`, `update`, `document add` / `document ls` (the owning ticket), `document show` / `document put` / `document rm` (`--ticket`). |
| `ambiguous_id`          | Strict-resolved partial id matched >1 ticket.                                                   | `candidates: string[]`       | Same set as `not_found` plus `dep tree`.                                                                                                  |
| `doc_not_found`         | A document selector matched no document. Separate from `not_found` so the reader is not sent looking for a ticket. | —                            | `document show`, `document put`, `document rm`.                                                                                           |
| `ambiguous_doc`         | A document selector matched more than one document. Never resolved to a first match.            | `candidates: string[]`       | `document show`, `document put`, `document rm`.                                                                                           |
| `invalid_doc_type`      | A document write named a type outside `.knot.edn`'s `:doc-types`. Nothing is written.           | `value: string`, `allowed: string[]` | `document add`, `document put`. Also a `check` code — see *`check` shape*.                                                        |
| `cycle`                 | `dep <from> <to>` would create a cycle.                                                         | `cycle: string[]` (the path) | `dep`.                                                                                                                                    |
| `has_incoming_refs`     | `delete` target is referenced by another ticket through `:parent` / `:deps` / `:links`, or owns documents. | `referrers: {id, field}[]`, `documents: string[]` | `delete` without `--cascade`. Drop the refs first (`undep`, `unlink`, `update --parent ""`), remove the documents (`document rm`), or re-run with `--cascade`. |
| `invalid_argument`      | Validation failure on a flag value or flag combination.                                         | —                            | `info --json`, `check --json` (any parse error); `update --json` (conflicting body flags); `close` / `status` / `update` (`--force` with a blank `--summary` while a gate fires on a terminal target); `document add` (missing `--title`) and `document put` (missing `--title` or `--type` — the replace is total, so neither is carried over). Other commands keep argument-parse errors on stderr — see below. |
...
| `missing_required_docs` | A transition into a status listed in `.knot.edn`'s `:required-docs`, where the ticket does not own a document of every type that status requires. | `missing_doc_types: string[]`, `target: string` | `start`, `status`, `update --status`. Attach the documents, or pass `--force`. Only the missing types are listed, not every required one.  |
...
| `document put <sel>`    | `{id, ticket, title, type, created, updated}`                                                  | no    | Post-replace metadata. `created` is preserved; `updated` bumps.                                                                                                                                                                                                                                                                                                                                             |
| `document rm <sel>`     | `{deleted: {id, path}}`                                                                        | n/a   | `path` is where the file *was*.                                                                                                                                                                                                                                                                                                                                                                             |
| `document ls <ticket>`  | `{ticket, documents}`                                                                          | no    | `documents` is always present, `[]` when the ticket owns none; entries are `{id, ticket, title, type, created, updated}`.                                                                                                                                                                                                                                                                                    |
