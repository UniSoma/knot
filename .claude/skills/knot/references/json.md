...
validators add `field` and `value`. Exactly four codes add `path` (absolute, POSIX-separated — see *Path fields*):
`terminal_outside_archive`, `frontmatter_parse_error` (which also interpolates it into `message`; read `path`, not
`message`), `missing_required_field` when the missing field is `id`, and `skill_stale`, whose `path` is the `SKILL.md`
and whose `value` is always present: the stamped version, or `null` when the stamp is missing or unreadable. Treat
`path` as optional and branch on its presence.
