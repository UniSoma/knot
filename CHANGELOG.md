...
- **`install.sh` and `install.ps1` install the binary in one line.** They pick the asset for the machine, check it against `SHA256SUMS`, and install to `~/.local/bin` or `%LOCALAPPDATA%\Programs\knot`. `KNOT_VERSION` pins a release, `KNOT_INSTALL_DIR` changes the target, and `KNOT_RELEASE_URL` points at another releases base. On an Apple silicon Mac running under Rosetta, `install.sh` still picks the arm64 build.
- **`bb build:release [--target all|host|<os>-<arch>]` builds the release assets into `dist/`.** It downloads babashka at the version in `.bb-version`, checks each download against upstream's sha256, and keeps the unpacked binaries under `target/release/<target>/`.
...
- **`knot skill install [dir]` writes the agent skill out of the CLI.** The command writes `SKILL.md`, `references/` and `agents/openai.yaml` into `.claude/skills/knot` under the project root, where Claude Code loads them from. Pass a directory to install anywhere else, or set `:skill-dir` in `.knot.edn` to change the default; relative paths resolve from the project root and `~` expands. An explicit directory that differs from that default draws a one-line stderr hint suggesting `:skill-dir`, echoing the path as typed, so a machine-specific absolute path does not land in a committed config by accident. Install never writes `.knot.edn` itself. Every run overwrites the seven files knot owns and touches nothing else in the target, so re-running the command is how a project takes a newer version of the skill. The installed `SKILL.md` carries an `<!-- installed by knot <version> -->` comment after its frontmatter. `--json` returns `{dir, files}`, and `knot info` gained `paths.skill_dir` (the configured value, `null` when unset) and `paths.skill_path`.

- **`knot prime`'s closing pointer is live.** The primer used to end both preambles by telling the agent to invoke the `knot` skill whether or not one was installed. It now searches `:skill-dir`, then `<project-root>/.claude/skills/knot`, then `~/.claude/skills/knot` for a `SKILL.md`: found, and the sentence is unchanged; not found, and the same list of judgment areas routes to `knot help topics`, followed by a line nudging `knot skill install`. Each preamble keeps its own branch list. `--json` carries `skill_installed` and `skill_dir`, the directory whose `SKILL.md` answered the search, so an orchestrator can act on the state. Note that `prime --json`'s `skill_dir` is that resolved directory while `info --json`'s `paths.skill_dir` echoes the configured key; the `json` topic's path table spells out the difference.

- **`knot check` and `knot prime` warn when the installed skill is stale.** Both read the `<!-- installed by knot <version> -->` stamp back out of `SKILL.md`. A missing or unreadable stamp counts as stale, and so does any version other than the running CLI's, older or newer. `knot check` looks only at the project copy (`:skill-dir`, else `.claude/skills/knot`) and reports a `skill_stale` warning whose `path` is the `SKILL.md` and whose `value` is the stamp or `null`. The warning ends with "run `knot skill install` and commit" and never changes the exit code. `knot prime` checks whichever copy its pointer found, home copy included, and adds one line under "invoke the `knot` skill" naming both versions. For a home copy, that line says to run `knot skill install ~/.claude/skills/knot`. `prime --json` gains `skill_stale` and `skill_version`.

### Changed

- **The agent skill has one source, `resources/knot/skill/`, and the CLI serves it both ways (ADR 0019).** The three files under `references/` were reorganised into five topic-named guides, and the "Notes and revisions" and "Working autonomously" blocks moved out of `SKILL.md` into two of them. Those same files are what `knot help <topic>` prints and what `knot skill install` writes, so a change to the judgment is one edit that reaches pull and pointer together. The copy committed at `.claude/skills/knot/` stays, regenerated by the command and pinned by a test that compares both trees byte for byte apart from the version stamp. `references/json-protocol.md` is now `references/json.md`; `lifecycle-gates.md` and `listing-filters-and-columns.md` are `lifecycle.md` and `graph.md`. ADR 0019 supersedes three clauses of ADR 0017: the definition of the pull surface, its rejection of a CLI-produced pointer, and its deletion of the shared pointer helper.

### Fixed

- **`bb build:jolt` builds again against jolt 0.8.5.** Since jolt 0.8.1 a host class comes from whichever library declares it, so the pinned `jolt-lang/time` commit no longer supplied `DateTimeFormatter` and the build died inside `data.json`. The pin moves to `v0.0.9`, which declares its classes. `jolt/README.md` is refreshed against 0.8.5: the `babashka.fs/list-dir` divergence is fixed upstream, the binary runs 20 to 40% faster than the August figures, and `--tree-shake` is silently skipped because `start-server!` and `flatland.ordered.set` resolve vars at runtime.
...
- **`knot close <id> --external-ref git:<sha>` records the commit as part of the close.** The flag is repeatable and appends: existing references stay, the new one lands at the end, and re-recording one already on the ticket changes nothing. The status change, the `--summary` note and the reference are written once, so the commit that finished a ticket no longer has to go into the summary prose or into a second `knot update` call that replaced the whole reference list. `git:<sha>` is a convention, not a validated format — knot stores the string verbatim, never parses it, and never fills it in from `git HEAD`, because most closes are not paired with a commit.

- **`knot update --add-external-ref` / `--remove-external-ref` apply one-reference deltas.** Both are repeatable and idempotent: adding a reference already present, or removing one that is absent, leaves the list as it was. They are mutually exclusive with the replace-all `--external-ref`, which exits 1 with `invalid_argument` — the same shape `--tags` versus `--add-tag` / `--remove-tag` already had.

- **`knot prime --parent <id>` scopes the whole primer to one umbrella.** Every primer section — In Progress, Ready to close, Ready, Recently Closed — narrows to the direct children of the given id, so an agent working a wave starts its session seeing only that wave. The flag is repeatable, resolves partial ids, and composes with the filters `prime` already took (`--mode afk --parent kno-01abc`). The umbrella itself is excluded: it is not its own direct child. One asymmetry is deliberate — on `list`/`ready`/`blocked`/`closed` an unresolvable `--parent` exits 1, but `prime` is wired to `SessionStart` and must never fail a session, so it degrades to the no-project primer at exit 0.

- **`knot help list`, `ready` and `blocked` define the computed columns.** Each of the three now carries a NOTES block naming `AGE`, `AC`, `CHLD`, `LEV`, `CPL` and `CC`, what each one counts, and the `--json` field that carries it (`acceptance`, `children_total`/`children_terminal`, `leverage`, `coupling`, `cc`; `AGE` has none and is bucketed from `updated`). The shared live-induced scope leads the block, so the rule that a closed ticket neither counts nor conducts is stated once. The definitions used to live only in `CONTEXT.md`, the ADRs, and the bundled skill, which put them out of reach of anyone holding the CLI — help is the surface that ships with the tool. The skill's listing reference keeps only the judgment help cannot carry: which number to act on when several tickets are ready, and what a high coupling count costs a cold agent.
