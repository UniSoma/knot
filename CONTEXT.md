...
**Derived section**:
One of the five sections `knot show` prints that no body holds: `## Acceptance Criteria` comes from the `acceptance` field, `## Blockers` and `## Linked` from `deps` and `links`, and `## Blocking` and `## Children` from other tickets' `deps` and `parent` read backwards. `knot show` synthesizes them at render time, after a blank line and under an HTML comment naming the source field and the flag that writes it; `--json` carries the same five as structured keys and keeps `sections` body-only. A render is therefore not a round trip: the same heading written back into the body is a second copy that drifts from the field the moment the field moves, which is why the write surface refuses the five names and `check` flags the ones already on disk.
_Avoid_: "the acceptance section" for something stored (the checklist is display-only — the criteria live in frontmatter); "computed section" or "inverse section" in prose (both are the code's names, and *inverse* covers only the four graph sections, not the checklist); treating a `show` render as a body you can paste back.
