...
> *v0.3:* `dep cycle` was removed (`knot check --code dep_cycle` subsumes
> it; whole-project scan including archive). `info`, `update`, and `check`
> were added to the surface. `--json` is also accepted on every mutating
> command. See `CHANGELOG.md` for the full surface-area delta.
...
> *v0.3:* `--afk` and `--hitl` were removed (kno-01kqgqa7wnep). `--mode <value>`
> is the only path to set mode at create time; the shortcuts assumed canonical
> mode names and broke for projects that customize `:modes`. Migration:
> `--afk` → `--mode afk`, `--hitl` → `--mode hitl`. `--acceptance` is now a
> repeatable string flag that appends structured frontmatter entries
> (`{title, done: false}`); the `## Acceptance Criteria` body section is
> synthesized at display time and never stored on disk.
